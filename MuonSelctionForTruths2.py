...
savelocat ='/n/holystore01/LABS/guenette_lab/Users/lrogers/PredictionStuff/data/SmallerSphere2/SelectionCuts'
...
#MC muon data                                                                                                              
locatformat='/n/holystore01/LABS/guenette_lab/Users/lrogers/PredictionStuff/data/SmallerSphere2/esmeralda/Output/'
num =0
probnum=0
ttlevts=0
...
for i in range(start_run, end_run):
    ...
    run_num=str(int(i)).zfill(4)
    locat=locatformat
    fileformat=locat+'esmeralda-{}-MUONS.h5'
    ...
    try:
        ...
        evtslow=pd.read_hdf(files,key='/CHITS/lowTh')
        lstevts=evtslow.event.unique()
        #print(MC_info, files)
    ...
    for evts in events_data.event.unique():
        ...
        locofMCevt=np.where(lstevts==evts)[0][0]
        mixi=MC_particles[(MC_particles.event_id==locofMCevt) & (MC_particles.particle_id==1)]



        
        if len(mixi)==0:
            print("empty file, run",i,evts)
            continue
        ...
        events_dataplt['Ec']=events_dataplt['Ec']*3.1
        ...
        if  (EmxEtl>.1) or (FIDcut==False):
            #print('EmxEtl is huge or ttlnrg is low',EmxEtl,ttl_nrg,i,evts)
            ...
            mu_theta.append(NaN)
            mu_phi.append(NaN)
            xint.append(xintercept)
            yint.append(yintercept)
            zint.append(zintercept)
            truth_theta.append(MCtheta[0])
            truth_phi.append(MCphi[0])
            truth_per.append(NaN)
            truth_dedx.append(NaN)
            truth_len.append(NaN)
            FidCross.append(FIDcut)
            Pass.append(False)
            Weight.append(NaN)
        ...
        Rsp=1000
        xmu0=mixi.initial_x.values
        ymu0=mixi.initial_y.values
        zmu0=mixi.initial_z.values
        xmu1=mixi.final_x.values
        ymu1=mixi.final_y.values
        zmu1=mixi.final_z.values
        A = (xmu0)**2 + (ymu0)**2 + (zmu0-250)**2 - Rsp**2
        C = MCdx**2 + MCdy**2 + (MCdz)**2
        B = 2*(xmu0*MCdx+ymu0*MCdy+zmu0*MCdz)

        rp=np.roots([A[0],B[0],C[0]])

        tsphere=np.max(rp)
        xsph=tsphere*(xmu1-xmu0)+xmu0
        ysph=tsphere*(ymu1-ymu0)+ymu0
        zsph=tsphere*(zmu1-zmu0)+zmu0

        
        magsphere=np.sqrt(xsph**2+ysph**2+zsph**2)
        sphunix=xsph/magsphere
        sphuniy=ysph/magsphere
        sphuniz=zsph/magsphere

        W=4*(sphunix*MCunix+sphuniy*MCuniy+sphuniz*MCuniz)
        ...
        events_dataplt['MCcross']=Distance((events_dataplt.X-Xint),(events_dataplt.Z-Zint),(events_dataplt.Y-Yint),Rx(1,MCphi,MCtheta),Rz(1,MCphi,MCtheta),Ry(1,MCphi,MCtheta))
        ...
        if nrgonline==0:
            print ("nrg messed up", evts)

        #print("got to where should be appending",i,ttl_nrg,len(dedxall))
        ...
        xint.append(Xint)
        yint.append(Yint)
        zint.append(Zint+depth/2)
        truth_theta.append(float(MCtheta[0]))
        truth_phi.append(float(MCphi[0]))
        truth_per.append(MCper)
        truth_dedx.append(MCdedx)
        truth_len.append(MClen)
        FidCross.append(True)
        Pass.append(False)
        Weight.append(W)
        zvar.append(np.var(events_dataplt.Z))
    ...
    pd.DataFrame({'dEdx':dedxall,
                  ...
                  'MCdEdx':truth_dedx,
                  'MClength':truth_len,
                  'FiducialPass':FidCross,
                  'zvariance':zvar,
                  'Passed':Pass, #will need to update this outside of here
                  'Weight':Weight,
